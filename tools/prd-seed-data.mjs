...
export const reqpub = {
  ...
  scalars: {
    ...
    verify_note: 'A capability is accepted when it passes its fit criterion in production and is covered by the automated suite (192 unit tests, 290 backend checks on a real Postgres). The SOC 2 requirement is accepted only when independently audited; the sealing requirement only when a sealed export verifies against its exact baseline independently of the platform.',
  ...
  rows: {
    ...
    nfr: [
      ...
      { stmt: 'Every change ships with regression tests and the suite stays green (192 unit, 290 backend checks).', fit: 'CI runs both suites on every push. Test.', pri: 'Must', comp: 'Relational core' },
