...
1. Pure document pipeline: `tests/domain.test.mjs` (13: builders, IDs, diffing, brief redaction, summary math), `tests/share.test.mjs` (8: section-scoped share payloads), `tests/msgdedup.test.mjs` (5: optimistic/realtime message dedupe).
...
`npm test` runs layers 1 and 2 with node only (38 checks). `npm run test:backend` runs layer 3 (148) on an embedded Postgres. Total: 186.
